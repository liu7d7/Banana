...
public class NukerBypass extends Feature {
        ...
        switch (PlayerUtils.GetFacing()) {
            case East:
                for (int i = 0; i < forward.getValInt(); i++) {
                    mine(mc.player.getBlockPos().east(i));
                    for (int i2 = 0; i2 < up.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().east(i).up(i2));
                    }
                    for (int i2 = 0; i2 < left.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().east(i).north(i2));
                        for (int i3 = 0; i3 < up.getValInt(); i3++) {
                            mine(mc.player.getBlockPos().east(i).north(i2).up(i3));
                        }
                    }
                    for (int i2 = 0; i2 < right.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().east(i).south(i2));
                        for (int i3 = 0; i3 < up.getValInt(); i3++) {
                            mine(mc.player.getBlockPos().south(i2).east(i).up(i3));
                        }
                    }
                }
                for (int i = 0; i < backward.getValInt(); i++) {
                    mine(mc.player.getBlockPos().west(i));
                    for (int i2 = 0; i2 < up.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().west(i).up(i2));
                    }
                    for (int i2 = 0; i2 < left.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().west(i).north(i2));
                        for (int i3 = 0; i3 < up.getValInt(); i3++) {
                            mine(mc.player.getBlockPos().west(i).north(i2).up(i3));
                        }
                    }
                    for (int i2 = 0; i2 < right.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().west(i).south(i2));
                        for (int i3 = 0; i3 < up.getValInt(); i3++) {
                            mine(mc.player.getBlockPos().west(i).south(i2).up(i3));
                        }
                    }
                }
                break;
            case South:
                for (int i = 0; i < forward.getValInt(); i++) {
                    mine(mc.player.getBlockPos().south(i));
                    for (int i2 = 0; i2 < up.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().south(i).up(i2));
                    }
                    for (int i2 = 0; i2 < left.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().east(i2));
                        for (int j = 0; j < up.getValInt(); j++) {
                            mine(mc.player.getBlockPos().south(i).east(i2).up(j));
                        }
                    }
                    for (int i2 = 0; i2 < right.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().west(i2));
                        for (int j = 0; j < up.getValInt(); j++) {
                            mine(mc.player.getBlockPos().west(i2).south(i).up(j));
                        }
                    }
                }
                for (int i = 0; i < backward.getValInt(); i++) {
                    mine(mc.player.getBlockPos().north(i));
                    for (int j = 0; j < up.getValInt(); j++) {
                        mine(mc.player.getBlockPos().north(i).up(j));
                    }
                    for (int i2 = 0; i2 < left.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().east(i2));
                        for (int j = 0; j < up.getValInt(); j++) {
                            mine(mc.player.getBlockPos().north(i).east(i2).up(j));
                        }
                    }
                    for (int i2 = 0; i2 < right.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().west(i2));
                        for (int j = 0; j < up.getValInt(); j++) {
                            mine(mc.player.getBlockPos().north(i).west(i2).up(j));
                        }
                    }
                }
                break;
            case West:
                for (int i = 0; i < forward.getValInt(); i++) {
                    mine(mc.player.getBlockPos().west(i));
                    for (int j = 0; j < up.getValInt(); j++) {
                        mine(mc.player.getBlockPos().west(i).up(j));
                    }
                    for (int i2 = 0; i2 < left.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().south(i2));
                        for (int j = 0; j < up.getValInt(); j++) {
                            mine(mc.player.getBlockPos().west(i).south(i2).up(j));
                        }
                    }
                    for (int i2 = 0; i2 < right.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().north(i2));
                        for (int j = 0; j < up.getValInt(); j++) {
                            mine(mc.player.getBlockPos().west(i).north(i2).up(j));
                        }
                    }
                }
                for (int i = 0; i < backward.getValInt(); i++) {
                    mine(mc.player.getBlockPos().east(i));
                    for (int j = 0; j < up.getValInt(); j++) {
                        mine(mc.player.getBlockPos().east(i).up(j));
                    }
                    for (int i2 = 0; i2 < left.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().east(i).south(i2));
                        for (int j = 0; j < up.getValInt(); j++) {
                            mine(mc.player.getBlockPos().south(i2).east(i).up(j));
                        }
                    }
                    for (int i2 = 0; i2 < right.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().east(i).north(i2));
                        for (int j = 0; j < up.getValInt(); j++) {
                            mine(mc.player.getBlockPos().east(i).north(i2).up(j));
                        }
                    }
                }
                break;
            case North:
                ...
                for (int i = 0; i < backward.getValInt(); i++) {
                    mine(mc.player.getBlockPos().south(i));
                    for (int j = 0; j < up.getValInt(); j++) {
                        mine(mc.player.getBlockPos().south(i).up(j));
                    }
                    for (int i2 = 0; i2 < left.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().west(i2));
                        for (int j = 0; j < up.getValInt(); j++) {
                            mine(mc.player.getBlockPos().south(i).west(i2).up(j));
                        }
                    }
                    for (int i2 = 0; i2 < right.getValInt(); i2++) {
                        mine(mc.player.getBlockPos().east(i2));
    ...
    public void onMove(final MotionUpdateEvent event) {
        ...
        poses = getBlacklistedBlockPoses();

        getBlocks();
    }

    private void mine(final BlockPos pos) {
        if (!poses.contains(pos) || !disable.getValBoolean())
